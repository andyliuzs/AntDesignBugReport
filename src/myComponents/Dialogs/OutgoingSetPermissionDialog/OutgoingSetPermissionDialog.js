...
@Form.create()
class OutgoingSetPermissionDialog extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (current && current.acl_status == OutgoingPermissionType.Control.key) {
      this.fetchAllPeers()
    }
  }

  fetchAllPeers = () => {
    console.log('fetchAllPeers')
    const {dispatch} = this.props
    const {current} = this.state
    dispatch({
      type: DISPATCH.outgoingManage.getPeers,
      payload: { target_type: DeviceType.ServerDeviceType.key,
        target_id: current.server_id,},
      callback: (res) => {
        console.log('fetchAllPeers', res)
  ...
  handleSubmit = e => {
    ...
    validateFields((err, values) => {
      ...
      if (!err) {
        ...
        if (current.acl_status == OutgoingPermissionType.Control.key) {
        ...
        if (dispatch) {
          dispatch({
            type: DISPATCH.outgoingManage.updatePermission,
            payload: {...values},
            callback: (res) => {
              if (res.r === ResponseDataResult.OK) {
                onSuccess();
  ...
  onSelChange = (value) => {
    ...
    if (value == OutgoingPermissionType.Control.key) {
      this.fetchAllPeers()
    } else {
      this.setState({
        targetKeys: [],
  ...
  render() {
    ...
    const okBtnText =  formatMessage({id: 'app.cb.globalApp.modify'})
    const modalFooter = {
        ...
        <div>
          <Descriptions size="small" column={1}>
            <Descriptions.Item
              ...
              span={1}
            >
              <Select width={'160px'} dropdownMatchSelectWidth={false} defaultValue={current.acl_status} placeholder={formatMessage({id: 'app.cb.globalApp.pleaseSel'})}
                      onChange={this.onSelChange}>
                <Option value={OutgoingPermissionType.Open.key}>{OutgoingPermissionType.Open.desc}</Option>
                <Option value={OutgoingPermissionType.Block.key}>{OutgoingPermissionType.Block.desc}</Option>
                <Option value={OutgoingPermissionType.Control.key}>{OutgoingPermissionType.Control.desc}</Option>
              </Select>
            </Descriptions.Item>
          </Descriptions>
...
OutgoingSetPermissionDialog.propTypes = {
  onDismiss: PropTypes.func,
  onSuccess: PropTypes.func,
  removeDialog: PropTypes.func.isRequired,
};
OutgoingSetPermissionDialog.defaultProps = {
  onDismiss: () => {
...
export default connect(mapStateToProps)(OutgoingSetPermissionDialog)
